    ...
    if (!has_base && has_index) {
        ...
        if (shift) {
            ...
            emit_bitfield(result.insn_buf, is_64bit, /*opc=*/2 /*UBFM*/,
                          ...
                          /*imms=*/static_cast<int8_t>(reg_width - shift - 1), index_idx,
                          result_reg);
            return result_reg;
        }
...
auto compute_operand_address(TranslationResult& result, int is_64bit, IROperand* op, int dst_reg)
    ...
    if (op->mem.seg_override) {
        ...
        if (seg_override == 1) {
            ...
            result._fixups.push_back(
                Fixup{.kind = FixupKind::Branch26,
                      .insn_offset = static_cast<uint32_t>(result.insn_buf.end),
    ...
    if (kind == IROperandKind::Immediate) {
        ...
        result.external_fixups.push_back(
            Fixup{.kind = FixupKind::Arm64Page21,
                  .insn_offset = static_cast<uint32_t>(result.insn_buf.end),
                  .target = static_cast<uint32_t>(fixup_target)});
        emit_adr(result.insn_buf, /*is_adrp=*/1, out_reg, 0);

        // Emit ADD out_reg, out_reg, #<page_offset>   [patched by Arm64PageOffset12]
        result.external_fixups.push_back(
            Fixup{.kind = FixupKind::Arm64PageOffset12,
                  .insn_offset = static_cast<uint32_t>(result.insn_buf.end),
                  .target = static_cast<uint32_t>(fixup_target)});
...
auto read_operand_to_gpr(TranslationResult& result, bool is_64bit, IROperand* operand,
    ...
    if (operand->kind == IROperandKind::Register) {
        return translate_gpr(&result, static_cast<int>(is_64bit), operand->reg.reg.value,
                             extend_mode, hint_reg);
    }

    if (operand->kind == IROperandKind::BranchOffset) {
        return emit_load_immediate(result, is_64bit, static_cast<uint64_t>(operand->branch.value),
                                   hint_reg);
    }

    if (operand->kind == IROperandKind::Immediate) {
        return emit_load_immediate(result, is_64bit, static_cast<uint64_t>(operand->imm.value),
                                   hint_reg);
    }
    ...
    if (operand->kind == IROperandKind::MemRef) {
        ...
        const int addr_reg =
            compute_operand_address(result, static_cast<int>(addr_size == IROperandSize::S64),
        ...
        const uint32_t natural_size = is_64bit ? static_cast<uint32_t>(IROperandSize::S32)
                                               : static_cast<uint32_t>(IROperandSize::S16);
    ...
    if (operand->kind == IROperandKind::AbsMem) {
        ...
        const int addr_reg =
            compute_operand_address(result,
...
//
// Layout (fixed offsets, same style as the RC dispatch chains).  Every
// branch goes forward: Rosetta's state recovery walks the translation's
// control flow when a signal lands in it and treats any backward edge as a
// loop, which it refuses when the body writes guest state.  The NaN path
// therefore carries its own copy of the MOVZ instead of jumping back to [3].
...
//   [2] CBZ  Xt, +7 → [9]  (S16/S32: fits)   ; CBNZ for S64 (not MAX)
//   [3] MOVZ Xd_int, #indef                  ; out of range
//   [4] B    +5 → [9]
//   [5] FCMEQ Dscr, Dd_val, Dd_val           ; 0 iff NaN
//   [6] FMOV Xt, Dscr
//   [7] CBNZ Xt, +2 → [9]                    ; not NaN: keep the 0
//   [8] MOVZ Xd_int, #indef                  ; NaN → indefinite
//   [9] done
//
...
auto emit_fist_indefinite_guard(TranslationResult& result, AssemblerBuffer& buf, int Xd_int,
    ...
    // width's integer indefinite, one instruction for every width
    const auto emit_indefinite = [&] {
    ...
    // [2] in-range test → [9].  S16/S32: diff==0 ⇔ fits.  S64: diff!=0 ⇔ not MAX.
    emit_cbz(buf, /*is_64bit=*/1, /*is_nz=*/size == IROperandSize::S64 ? 1 : 0, Xt, 7);
    // [3] out of range
    emit_indefinite();
    // [4] B +5 → [9]
    emit_b(buf, 5);
    ...
    // [7] CBNZ Xt, +2 → [9]  (not NaN: the 0 stands)
    emit_cbz(buf, /*is_64bit=*/1, /*is_nz=*/1, Xt, 2);
    // [8] NaN → the indefinite
    emit_indefinite();
    // [9] done
