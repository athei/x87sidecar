...
auto Translator::translate_instruction(TranslationResult* translation_result, IRBlock* block,
    ...
    // dirty run.  Flush-when-clean is a no-op (allocates no scratch).
    {
        ...
        if (!ir_disabled && cache.active() && cache.run_remaining >= 3) {
            // Note: previously also gated on the four cache dirty flags
            // (top_dirty / tag_push_pending / deferred_pop_count / perm_dirty).
            // emit_x87_cache_flush inside X87IRLower::lower() now writes those
            // back at the start of lowering, letting IR run on dirty-state runs.
