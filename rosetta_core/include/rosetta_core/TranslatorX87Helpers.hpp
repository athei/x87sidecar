...
// =============================================================================
// Cache state pre-flush — make memory canonical before X87IR::compile_run.
//
// X87IR rebuilds the x87 stack model from scratch and assumes the in-memory
// state is canonical at the run start (no deferred TOP, tag, perm, or pop
// updates).  Without this helper the IR pipeline gates itself out whenever
// the cache's deferred-state flags are set — pushing the run onto the
// peephole/single-op fallback path.
//
// This helper emits exactly the flushes that x87_end's late-flush path would
// have emitted at run end, but eagerly, before IR builds its SSA.  Each step
// is gated on its own dirty flag so flush-when-clean is a no-op.
//
// Resets the four cache flags (top_dirty / tag_push_pending /
