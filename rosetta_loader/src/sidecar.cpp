...
// Body of a runtime transcendental IPC request (msgh_id=0x10000002).
// 24 bytes: opcode_tag (u32) + pad (u32) + in0 (f64) + in1 (f64).
struct TranscendentalRequest {
    uint32_t opcode_tag;
    uint32_t pad;
    double   in0;
    double   in1;
...
// Reply body: 16 bytes, two doubles.
struct TranscendentalReply {
    double out0;
    double out1;
};

// Compute the transcendental.  Logs a one-shot warning to stdout when a
// known-simplification path fires (out-of-range trig input, fprem
// iterative-reduction skip, etc.) so a future bug report from a real
// workload tells us "your simplification bit me."  See plan
// "Known simplifications".
void runTranscendental(const TranscendentalRequest& req,
                       TranscendentalReply* out) {
    using namespace rosetta_core;

    auto warn_oor_trig = [](const char* op, double v) {
        static std::atomic<bool> warned{false};
        if (!warned.exchange(true)) {
            fprintf(stdout,
                    "[rosettax87] %s simplified — input |ST(0)|>=2^63 "
                    "(would set C2; computing via libm anyway); ST(0)=%a\n",
                    op, v);
        }
    };

    auto trig_in_range = [](double v) {
        // x86 spec: fsin/fcos/fptan set C2=1 (and ST(0) unchanged) when
        // |ST(0)| >= 2^63.  We always compute via libm regardless.
        return !(std::fabs(v) >= 9.2233720368547758e18);  // 2^63
    };

    out->out0 = 0.0;
    out->out1 = 0.0;
    switch (req.opcode_tag) {
        case kTransF2xm1:
            out->out0 = std::exp2(req.in0) - 1.0;
            break;
        case kTransFsin:
            if (!trig_in_range(req.in0)) warn_oor_trig("fsin", req.in0);
            out->out0 = std::sin(req.in0);
            break;
        case kTransFcos:
            if (!trig_in_range(req.in0)) warn_oor_trig("fcos", req.in0);
            out->out0 = std::cos(req.in0);
            break;
        case kTransFsincos:
            if (!trig_in_range(req.in0)) warn_oor_trig("fsincos", req.in0);
            out->out0 = std::sin(req.in0);
            out->out1 = std::cos(req.in0);
            break;
        case kTransFptan:
            if (!trig_in_range(req.in0)) warn_oor_trig("fptan", req.in0);
            out->out0 = std::tan(req.in0);
            break;
        case kTransFpatan:
            // x86 fpatan = atan2(ST(1), ST(0)), pop.
            // JIT loads ST(0)→in0, ST(1)→in1; sidecar returns atan2(in1,in0).
            out->out0 = std::atan2(req.in1, req.in0);
            break;
        case kTransFyl2x:
            // x86: ST(1) * log2(ST(0)), pop.  in0=ST(0), in1=ST(1).
            out->out0 = req.in1 * std::log2(req.in0);
            break;
        case kTransFyl2xp1:
            out->out0 = req.in1 * std::log2(req.in0 + 1.0);
            break;
        case kTransFprem: {
            static std::atomic<bool> warned_prem{false};
            if (!warned_prem.exchange(true)) {
                fprintf(stdout,
                        "[rosettax87] fprem simplified — single-shot "
                        "fmod (real x87 is iterative); C2 forced to 0, "
                        "C0/C1/C3 quotient bits zeroed; in0=%a in1=%a\n",
                        req.in0, req.in1);
            }
            out->out0 = std::fmod(req.in0, req.in1);
            break;
        }
        case kTransFprem1: {
            static std::atomic<bool> warned_prem1{false};
            if (!warned_prem1.exchange(true)) {
                fprintf(stdout,
                        "[rosettax87] fprem1 simplified — single-shot "
                        "remainder (real x87 is iterative); C2 forced to "
                        "0, C0/C1/C3 quotient bits zeroed; in0=%a in1=%a\n",
                        req.in0, req.in1);
            }
            out->out0 = std::remainder(req.in0, req.in1);
            break;
        }
        default:
            // Unknown tag — leave outputs zero. Caller's emit shouldn't
            // reach here unless the tag enum is out of sync with this
            // switch.
            fprintf(stdout,
                    "[rosettax87] unknown transcendental tag %u\n",
                    req.opcode_tag);
            break;
    }
}
...
void runReceiveLoop(mach_port_t servicePort, mach_port_t parentTaskPort) {
    ...
    for (;;) {
        ...
        if (hdr->msgh_size >= 24 + sizeof(TranscendentalRequest) &&
            hdr->msgh_id == 0x10000002 && replyPort != MACH_PORT_NULL) {
            // Runtime transcendental request from JIT-emitted trampoline.
            TranscendentalRequest treq{};
            std::memcpy(&treq, buf.bytes + 24, sizeof(treq));
            TranscendentalReply trep{};
            runTranscendental(treq, &trep);

            struct TranscendentalReplyMsg {
                mach_msg_header_t hdr;
                TranscendentalReply body;
            } rep{};
            rep.hdr.msgh_bits =
                MACH_MSGH_BITS(MACH_MSG_TYPE_MOVE_SEND_ONCE, 0);
            rep.hdr.msgh_size = sizeof(rep);
            rep.hdr.msgh_remote_port = replyPort;
            rep.hdr.msgh_local_port = MACH_PORT_NULL;
            rep.hdr.msgh_id = 0x10000002;
            rep.body = trep;

            kern_return_t kr_send = mach_msg(
                &rep.hdr, MACH_SEND_MSG, sizeof(rep), 0, MACH_PORT_NULL,
                MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
            if (kr_send != KERN_SUCCESS) {
                fprintf(stdout,
                        "sidecar: trans #%llu reply send failed 0x%x %s\n",
